	...
	abs(player.x - x) <= 10 && !player.grappleActive && !player.isPushing)
{
	...
	if (state == ShipState.SaveAnim)
	{
		...
		if (saving < 0)
		{
			...
			items_amount_iterate(player.items_state, Item.Missile, 0.25, 0.05)
			items_amount_iterate(player.items_state, Item.SMissile, 0.25, 0.05)
			items_amount_iterate(player.items_state, Item.PBomb, 0.25, 0.05)
			saving++;
		}
		else if(hatchFrame <= 0)
		{
			if(saving == 0)
			{
				...
				items_set_item_to_max(player.items_state, Item.Missile)
				items_set_item_to_max(player.items_state, Item.SMissile)
				items_set_item_to_max(player.items_state, Item.PBomb)
